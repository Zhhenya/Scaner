...
public class Optimizer {
    ...
    public Triad[] optimize(boolean silent) throws Exception {
        ...
        for(int i = 0; i < aggregates.length; i++) {
            ...
            if(current.action == Action.proc) function = true;
            if(current.action == Action.endp) function = false;
    ...
    private boolean mergeAggregates(Reference ref, Aggregate agg, List<Reference> left, List<Reference> right) {
        if(ref instanceof TriadReference) {
            ...
            if(other == null || other.getClass() != agg.getClass()) return false;
    ...
    private void tryOptimize(Triad triad, int i) {
        ...
        for(int j = from; j >= 0; j--) {
            ...
            if(triads[j].transfer == Transfer.FUNCTION) break;
            
            /* Remove variable if assigned */
            if(triads[j].action == Action.assign) clone.remove(triads[j].ref1);
            
            /* Remove global variables if function call */
            if(triads[j].action == Action.call) clone.removeGlobalVariables();
            
            /* Stop search if possible intersection size one or zero */
            if(clone.size() <= 1) break;
            
            /* Check for loops */
            if(triads[j].transfer == Transfer.ELSE) loops++;
            if(triads[j].transfer == Transfer.IF) loops--;
            if(loops != 0) continue;
            
            /* Intersection check code */
            Aggregate check = aggregates[j];
            if(check == null || check.getClass() != agg.getClass()) continue;
    ...
    private <E> List<E> exclude(List<? extends E> from, List<? extends E> elements) {
        ...
        for(E element : elements)
            if(result.contains(element)) result.remove(element);
        return result;
    }
    ...
    private void rebuild(Triad triad, int i, int j, List<Reference> left, List<Reference> right, boolean reverse) {
        ...
        if(size < previous.size() && used[j]) return;
        ...
        for(int k = 0; k < size - 1; k++) triads[current.triads.get(k)] = new Triad(Action.skip);
        ...
        for(int index : previous.triads) aggregates[index] = buildAggregate(index);
        for(int index : current.triads) aggregates[index] = buildAggregate(index);
    ...
    private Pair<Reference, Boolean> buildTriads(List<Reference> left, List<Reference> right, List<Integer> indices,
        ...
        if(left.size() == 1 && right.size() == 0) return Pair.of(left.get(0), false);
        if(left.size() == 0 && right.size() == 1) return Pair.of(right.get(0), true);
        ...
        if(left.size() != 0) {
            for(Reference ref : left) actions.add(Pair.of(ref, a));
            for(Reference ref : right) actions.add(Pair.of(ref, b));
        } else {
            reverse = true;
            for(Reference ref : right) actions.add(Pair.of(ref, a));
        }
