...
import compiler.Reference.*;
import compiler.Triad;
import compiler.Triad.Action;
import compiler.analyze.SemanticAnalyzer;
import compiler.analyze.SemanticAnalyzer.Node;
import compiler.input.Type;
import compiler.translator.Storage.*;
import compiler.translator.TranslatorAction.*;
...
public class Translator {
    ...
    public Translator(Triad[] t, Diagrams analyzer) {
        ...
        for(Triad triad : triads) {
            if(triad.ref1 instanceof TriadReference) usages[((TriadReference)triad.ref1).index]++;
            if(triad.ref2 instanceof TriadReference) usages[((TriadReference)triad.ref2).index]++;
        }
    }
    ...
    public void translate(boolean silent) throws Exception {
        ...
        while(current != null) {
            if(current.global) global.add(current);
            if(current.lexeme != null && current.lexeme.type == Type.KeyMain) main = current;
        ...
        if(global.size() != 0) {
            addInstruction(".DATA");
            increaseOffset();
            for(Tree tree : global) addInstruction(String.format("%-12s", tree.node.), "DD");
            decreaseOffset();
            emptyLine();
        }
        ...
        for(int i = 0; i < triads.length; i++) {
            ...
            switch(triad.action) {
                ...
                case endp:
                    decreaseOffset();
                    addInstruction("endp");
                    if(i != triads.length - 1) emptyLine();
                    break;
                case add:
                case sub:
                case mul:
                ...
                case push:
                    ...
                    if(triad.ref1 instanceof TriadReference) decreaseUsage((TriadReference)triad.ref1);
                    break;
                case nop: {
                ...
                case jg: {
                    ...
                    if(triad.action == Action.jg) decreaseOffset();
        ...
        for(TranslatorAction action : actions) {
            if(action instanceof IncreaseOffset) offset++;
            if(action instanceof DecreaseOffset) offset--;
            
            if(action instanceof GenerateInstruction) {
                String instruction = ((GenerateInstruction) action).toString(offset);
                out.println(instruction);
                if(!silent) System.out.println(instruction);
            }
    ...
    private void buildBinaryOperation(int index, Triad triad) {
        ...
        if(triad.action == Action.assign) replaceLeft = true;
        ...
        if(triad.action == Action.assign && index != triads.length - 1) emptyLine();
        
        if(triad.ref1 instanceof TriadReference) decreaseUsage((TriadReference)triad.ref1);
        if(triad.ref2 instanceof TriadReference) decreaseUsage((TriadReference)triad.ref2);
        
        if(target != null) updateStorage(index, target);
        
        //System.out.println(Arrays.toString(registers));
    }
    
    private Storage convertImmediate(Storage storage) {
        if(!(storage instanceof Immediate)) return storage;
    ...
    private Storage getOperand(Reference reference) {
        if(reference instanceof ConstantReference) return new Immediate(((ConstantReference)reference).value);
        if(reference instanceof VariableReference) return new Memory(((VariableReference)reference).variable);
    ...
    private void updateStorage(int index, Storage storage) {
        ...
        if(storage instanceof Register) registers[((Register)storage).index] = index;
    }
    ...
    private void decreaseUsage(TriadReference ref) {
        ...
        if(usages[ref.index] == 0)
            if(storages[ref.index] instanceof Register) {
                ...
                if(mem.index != -1) temporary[mem.index] = -1;
            }
    }
    
    private Register getOrFreeRegister() {
        for(int i = 0; i < registers.length; i++)
            if(registers[i] == -1) return new Register(i);
        return getOrFreeRegister(0);
    }
    
    private Register getOrFreeRegister(int i) {
        Register reg = new Register(i);
        if(registers[i] == -1) return reg;
        
        Storage storage = null;
        for(int j = 0; j < registers.length && storage == null; j++)
            if(j != i && registers[j] == -1) storage = new Register(j);
        
        if(storage == null) storage = getFromTemporaryArea();
    ...
    private Memory getFromTemporaryArea() {
        for(int i = 0; i < temporary.length; i++)
            if(temporary[i] == -1) return new Memory(-(localStorage + (i + 1) * 4), i);
